...
`POST /api/v1/retrieve` and `GET /health` stay public so the embed keeps working
without credentials.
...
- **Public `/retrieve`.** See the route table. Admin auth does not protect
  knowledge-base reads by `company_id`.
